...
import pydantic.v1
pydantic.v1.BaseSettings = pydantic_settings.BaseSettings
sys.modules["pydantic.v1.BaseSettings"] = pydantic_settings.BaseSettings
# import chromadb
...
class ChromaDBService(BaseService): # Added (BaseService) back
    """Singleton-style service wrapping a ChromaDB Cloud collection."""

    def __init__(self, gemini_service: GeminiEmbeddingService = None) -> None:
        ...
        self.enabled = False # Added this line
    ...
    async def initialize(self) -> None:
        ...
        if not self._gemini or not self._gemini.is_initialized: # Modified condition
            logger.warning(
        ...
        try:
            ...
            # import chromadb # Moved import here to avoid import errors if not used
            ...
            # )
            self._client = None # Set client to None for no-op
            ...
            self._collection = None # Set collection to None for no-op

            await super().initialize()
            self.enabled = True # Set enabled to True if initialization "succeeds"
            logger.info(
